...
## SESSION 2026-09-18 — LAUNCH WEEK Gate 3 (Phase 3 question types, branch `launch/week-1`)
Phase 3 steps 3.1–3.8 committed (`a2ad0232`…`c016b883`). Gate 3 CLOSED except the full-canon backfill, which is blocked on a founder task (Anthropic credits), honestly recorded below.
- **DB bridge:** `scripts/verify-db-quiz-bridge.ts` extended with a 16-type fixture — **52/52 checks pass** (adapter + grader round-trip for every canonical type, wire formats incl. `tf_with_reason` half credit, ordering JSON array, matching JSON object, multiple_select comma-set, open types → pending).
- **Teacher-quiz e2e, all 16 types (live UI, prod test cohort):** seeded "Gate 3 — All Sixteen Types" via `tmp/gate3-seed-16type-quiz.ts` (RHET10). Penelope took it in the browser: every renderer mounted and accepted input; submit → **86% (12/22), 2 answers awaiting review** (reflection + free_response pending, never zeroed). Teacher results view shows the pending badges; override free_response to 3/4 → per-student and class average recompute (86%→83%).
- **Reader overlay e2e (jane-eyre, book-level quizzes):** Scholar trial 5/5 100% — vocabulary_in_context, passage question, matching (drag pairs), close_reading, tf_with_reason (two-part True/False + "AND WHY?" reason). Apprentice trial — multiple_choice, true_false, fill_blank (typed "Lloyd"), identification ("WHO SAID THIS?"), ordering ("DRAG TO REORDER"; deliberate wrong order → honest wrong-feedback). Screenshots captured. Cosmetic follow-up: ordering wrong-feedback prints the correct answer as a raw JSON array — Phase 9 polish candidate, not a grading bug.
- **SQL checks (prod):** all 13 platform types non-zero after the tier-1 run — mc 501, tf 427, fill_blank 416, vocab 396, identification 73, ordering 73, passage_id 73, matching 73, close_reading 143, tf_with_reason 73, theme_analysis 70, cross_reference 70, reflection 70. `books_full_quiz_set = 151` (target ≥ 1,270 NOT met — see blocker).
- **BLOCKER (founder task logged):** the full-canon `rebalance-types.ts --scope=all --commit` run died on Anthropic "credit balance is too low" 400s (~1,223 failures after 195 OK tier writes ≈ 65 books). Run killed; fully resumable via the `quiz_generation_runs` ledger. Resume command + ~$250–300 budget in `docs/LAUNCH-FOUNDER-TASKS.md`. `canon-quiz-validate.ts` random-25 deferred with it.
- **Docs:** NEW `docs/quiz-standard.md` (canonical 16-type vocabulary, one grader/renderer registry, 3×5 type ladder, validation, resume runbook); CLAUDE.md quiz-data line updated (old "36 chapter / 297 book / 99 books" figures superseded). `npm run typecheck` clean (only the known pre-existing registry.test.ts noise).

## SESSION 2026-09-18 — LAUNCH WEEK Gate 2 (Phase 2 billing verification, branch `launch/week-1`)
Phase 2 steps 2.1–2.11 committed (`45a9d822`…`3a1b4930`); this gate proves the launch billing model end-to-end.
- **Migration (applied to prod `vjaezrcuuzmbmnsfrtwt`):** `20260918020000_subscriptions_classroom_tier.sql` — the live `subscriptions_tier_check` only allowed solo/family/school (verified via pg_constraint), so every Classroom checkout's webhook write would have FAILED. Widened to `('solo','family','classroom','school')`. Real launch blocker caught by the gate.
- **`npm run verify:billing` = 155 checks, 0 failed** (against prod, ephemeral fixtures, full cleanup: 38 users + 3 classrooms). Extended with every brief-mandated scenario: free teacher (tools true, 30 students / 1 classroom); Classroom seats:25 (24→25 allowed, 26th rejected at `seat.used >= seat.allowance`, cancel-at-period-end retains, canceled reverts to free-teacher); School pooled seats:150 (two covered teachers, a student in two covered classrooms counts ONCE — used=3 not 4); School flat150; Family (allowance 4, classroom cap lifted); grandfathered Solo reads a beyond-sampler book (wuthering-heights); legacy per-teacher School row still resolves a teacher (allowance falls back to SCHOOL_MIN_SEATS=125).
- **`npm run verify:questions` (NEW script + npm alias) = 17 checks, 0 failed**: consume-beyond-balance raises `insufficient_questions`; refund restores + ledgers; replayed `stripe_event_id` grant is a NO-OP with exactly one ledger row; `refresh_question_grants` caps at accrual_cap (250+100→300) and never claws back an above-cap 500; non-owner consume → `not_pool_owner`, owner succeeds; authenticated callers cannot grant/refund (server-only surface). Safety: the cap test counts OTHER due pools first and skips if the global cron would touch real data (0 were due).
- **`verify-billing-stripe.ts` REWRITTEN** to the launch model (was the dead `STRIPE_PRICE_*` solo/monthly world): test clocks, classroom qty-12 annual lifecycle w/ renewal + metadata contract `{tier, student_seats}`, school qty-125 + flat300, family, top-up invoice. Skips cleanly (exit 0) — `.env.local` is missing 3 of 5 `TOME_PRICE_*` test ids (founder task added). Refuses non-test keys. `docs/billing-test-plan.md` Run 2 reworded to match what the script actually asserts (DB half = webhook's work → Run 3 + RPC tests).
- **Pricing parity:** `npm run test:billing:dollars` PASS; `/pricing` renders zero dollar literals and no $99/Family figure (all numbers derive from `src/lib/billing/config.ts` via `getBillingTiers()`, which excludes Family). `/homeschool` page does NOT exist yet — it is a **Phase 6.3 deliverable**; every Family CTA (`CheckoutButton`, profile upsell, tiers.ts, faqs) already points at `/homeschool` and derives $99 from `FAMILY_PRICE_USD_PER_YEAR`. The "/homeschool shows $99" half of the gate check is therefore deferred to Gate 6.
- **E2e trio:** server-side enforcement proven programmatically — createClassroom free-teacher gate (`src/lib/actions/classrooms.ts:54-66`, count≥allowance → "The free teacher plan includes one classroom…"), chip decrement path (`src/lib/credits/consume.ts` → `consume_question_credits`, math proven by verify:questions), school seat pooling (verify:billing). Browser confirmations recorded as a founder manual-QA task in `docs/LAUNCH-FOUNDER-TASKS.md`.

## SESSION 2026-09-18 — LAUNCH WEEK Phase 1 Gate 1 GREEN (branch `launch/week-1`)
Phase 1 (assignment loop through the REAL general reader) complete: steps 1.1–1.8 committed (`3478fa4e`…`ad02e342`); gate spec `e2e/assignment-loop.spec.ts` now **7/7 green** (`npx playwright test assignment-loop`, ~2.8m, serial). Coverage: teacher composes+publishes reading assignment with platform quiz (Apprentice) via the real composer UI → student `/assignments` canonical link (`links.ts` href asserted byte-exact) → ribbon (title · Chapters n–m · due) → mid-range CTA "Continue reading" → range-end `codex-pressable` "Begin Quiz" → Apprentice trial opens DIRECTLY (no difficulty drop-up), student answers from the DB answer key (MC + true/false + fill-blank renderers) → passes → full grading chain asserted in DB: `assignment_submissions` graded, `grades` row (score/max), `grade_history` mirror (keyed by `grade_id`, trigger-written), `classroom_gradebook` RPC row, `assignment_graded` notification. Plus: teacher-quiz mode (composer attach → QuizAttemptRunner → `teacher_quiz_results` + graded submission), no-quiz mode ("Mark as read" → full points), stale `?assignment=` for another book shows no ribbon.
**Honest notes / flake fixes that matter for humans too:**
- The chapter-end CTA label "Begin Quiz" ALSO renders when `assignmentCtx` hasn't loaded yet — clicking during that window launches the plain platform difficulty drop-up instead of the assignment quiz. The spec now waits for the assignment ribbon before clicking (`waitForRibbon`). Real students on slow connections could hit the same race; candidate Phase 9 polish: disable/neutral CTA until assignment ctx resolves when `?assignment=` is present.
- Reader resumes at saved `reading_progress` and this can override the `?ch=` intent — spec snapshots+clears Beatrice's the-odyssey progress in beforeAll (restored in afterAll).
- Reader first client render is occasionally lost to the known Liveblocks version-dupe SSR fallback (node 3.20.0 vs client 3.22.0) — spec reloads once; not a product fix.
- The persistent `codex-pressable` chapter-end CTA exists only in SCROLL mode; default prefs are "spread". Spec pins server `reading_preferences.prefs.mode='scroll'` (restored after) + localStorage seed.
- `classroom_gradebook` raises NOT_AUTHENTICATED for service-role (auth.uid() null) — the spec signs in as Hypatia with the anon key to call it (by design).
- No-quiz "Mark as read" finalize is an async server write — the spec polls `assignment_submissions` (up to 30s) instead of trusting a fixed wait.
- Quiz overlay feedback bar is a framer-motion spring → Playwright clicks flake; the spec advances via the overlay's keyboard shortcut (Enter). `grade_history` keys on `grade_id`, not submission_id. `teacher_quizzes` schema is flat (no `settings` json); `teacher_quiz_questions` uses `sort_order`.
Cleanup is idempotent: `[E2E-LOOP]`-prefixed assignments/quizzes swept in beforeAll and deleted in afterAll; quiz_results rows since run start deleted; prefs + progress restored. Gate 1 PASSED → proceeding to Phase 2 (billing config).

## SESSION 2026-09-18 — LAUNCH WEEK Phase 0 preflight (branch `launch/week-1` from main @ 813fcc28)
Executing the Launch-Week Master Brief v1.1. **Baseline (0.1):** removed `pnpm-lock.yaml`/`pnpm-workspace.yaml` (npm is canonical); `npm ci` clean; `npm run typecheck` fails ONLY in the known `src/components/sidebar/animations/__tests__/registry.test.ts` (TS2307 vitest + 8 implicit-any) — pre-existing, deferred to Phase 9; `npm run build` **exits 0** (1,900+ pages, known cover-route trace + og-image warnings); `npm run lint` crashes the node process mid-run (stack trace; last recorded healthy-ish run = 590 problems per AGENTS.md 2026-07-20) — pre-existing, not a gate.
**DB audit (0.2, read-only, vjaezrcuuzmbmnsfrtwt):** books=1280, with_chapters=1274, full_quiz_set=98, questions=1643 (4 types in use), authors=1, has_synopsis=105, has_cover=1264, painting_credit=3. Six chapterless: apology, discourse-on-method, in-a-glass-darkly, symposium, the-histories, utopia. Assignments by type: reading 4 (0 with quiz_id), essay 8, quiz 5 (3 with quiz_id), trial 1, discussion 1.
**Chapter convention (0.3): SETTLED = 0-based reader array positions, identity mapping.** `chapter_range_start/end` == reader `currentChapter` == `reading_progress.chapter_index`. Evidence in the header of NEW `src/lib/assignments/chapters.ts` (composer :104 comment + `c.number` options starting at 0 incl. front matter; grades.ts:166 predicate; prod row `2866ee1d-6f46-4c37-a386-ac4d20f5a535` "The Odyssey: Books I–III" start=1 end=3 with Preface at index 0). Helpers `toReaderIndex`/`fromReaderIndex` (identity), `isWithinRange`, `hasCompletedRange`; unit test `npm run test:assignments` = 4/4 green. All Phase 1 chapter arithmetic goes through these helpers only.
**0.4:** `docs/LAUNCH-FOUNDER-TASKS.md` created from Appendix A. Gate 0 PASSED.

## SESSION 2026-07-15 — Whole-book reader quiz pipeline build (gate 1 only; NO model call, NO DB write)
Built `scripts/quiz-gen/` for the subscription-only full-canon run. Live read-only audit against the exact Tome project `vjaezrcuuzmbmnsfrtwt`: 1,280 books; 1,274 with chapters; 6 `needs_ingestion`; 99 complete canonical whole-book sets; 1,176 incomplete books with chapters; 297 book-level quizzes / 1,485 questions; zero duplicate `(book_id,difficulty)` whole-book keys. The six ingestion gaps remain `apology`, `discourse-on-method`, `in-a-glass-darkly`, `symposium`, `the-histories`, and `utopia`.

Delivered deterministic HTML stripping/digests (including bounded head/middle/tail coverage for three substantial one-chapter books), fixed prompt + backend-compatible strict output schema, ChatGPT-authenticated Terra/Sol Codex worker isolation, semantic/structural validator, dual encoder, quota pacing/backoff, atomic progress, single-driver lock, scoped reports, and a service-role-only transactional RPC migration with a partial uniqueness guard. Gate state binds approval to the prompt/schema hash and enforces: prompt approval → automatic 2-book dry run → style approval → exact 10-book Terra pilot → STOP/report → explicit full-run approval → batches capped at 100. Active pilot/batch target ids and approved prompt/schema snapshots are immutable across restarts. Each rolling permit is consumed once; another requires the matching run's report plus its run-bound weekly-usage value. Pre-RPC insertion intents reconcile ambiguous crashes against the live DB without duplicate model work. Commit mode probes the exact versioned RPC without writing before it can spend model allowance.

Verification at this checkpoint: 27 isolated quizgen tests, strict quizgen typecheck, scoped ESLint, `git diff --check`, and the production Next.js build pass (the build retains unrelated pre-existing cover-path/OG edge warnings). The fail-closed dry-run smoke test stopped at the missing prompt approval. Prompt hash `b3dc5e6fbf16d010faafa887ed732f6480000e8d5ed0fba8a24a1100534f` and gate are intentionally unapproved. The migration `20260715145250_reader_quiz_batch_insert.sql` is authored but **not applied**. No `codex exec` generation worker ran, no quiz JSON was generated, and no Supabase mutation occurred. Resume only after Matthew reviews `scripts/quiz-gen/gen-task.md` and explicitly approves the prompt gate.

## SESSION 2026-07-15 — Quiz-system cleanup (Phases 1–4) BEFORE full-canon generation
Ran the 4-phase pre-generation cleanup on prod `vjaezrcuuzmbmnsfrtwt` (tome-app, NOT Anvil — confirmed). Guardrails honored: quarantine-not-delete (fully reversible), transactions, small batches, verify after each phase, project-scope checked before every write. Teacher_* tables untouched except the Phase-3 grading fix.

**PHASE 1 — READ-ONLY AUDIT (reader quizzes `quizzes`/`questions`):** Data is remarkably clean. Top-line coverage + 9 integrity checks (dupes, dual-encoding mismatch, per-type shape, difficulty casing, count drift, order integrity, orphans, unrenderable types, filler) surfaced only THREE real issues: (a) unrenderable question types, (b) quizzes on chapterless books, (c) filler explanations. Filler exported (NOT rewritten — that's the generation run's job) to `reports/legacy_filler_explanations.csv` = **214 rows**: 90 `exact_filler` ("See the text for the relevant passage and context." across 6 books r-u-r / tartuffe / the-consolation-of-philosophy / the-inspector-general / the-seagull / utopia, 15 Q each) + 124 `short_under40` (meaningful one-liners, 49 books).

**PHASE 2 — DATA FIXES (quarantine + recompute):** Created `_quarantine_quizzes` / `_quarantine_questions` (each with `quarantined_at`, `reason`). Quarantined **202 questions total** (187 unrenderable-type + 15 utopia chapterless) and **3 quizzes**; recomputed `question_count` for affected quizzes. Full re-audit came back ALL-ZERO for every check. FLAGGED for the generation run: 12 Master chapter quizzes (e.g. a-christmas-carol, moby-dick) now hold only 2–3 questions after unrenderable removal → regeneration targets.

**PHASE 3 — TEACHER-QUIZ GRADING FAIL-PATH (the one teacher_* exception):** Root-caused evidence quiz `cc29398a-fb4d-4a76-a905-9554dcc144ae` scoring 2/12 → two bugs: (i) an essay stored as a keyless `short_answer` fell through objective auto-grade to a phantom 0; (ii) a Virgil grade failure was recorded as graded 0. Smallest migration: extended `teacher_quiz_responses_graded_by_check` to include `'pending'` (no new column). Extracted the per-response decision into a NEW pure, server-only-free module `src/lib/teacher-quiz/objective.ts` (`resolveResponseGrade`, injectable grader) — the single home of the invariant: open-ended never auto-graded; grader throw/unparseable → pending (score=null, graded_at=null, NEVER 0); success always persists ai_feedback + ai_rubric_breakdown; keyless objective → pending. `grade.ts` now re-exports it (server-only + Anthropic stay isolated there). `submitQuizAttempt` routes every answer through it; results UI (`quiz-builder/[quizId]/results/page.tsx`) shows amber "Needs grading" for pending and blocks accidental-0 approval. `overrideResponseScore` already recomputes results from the full set (req #4, unchanged). Backfilled the **4 wrongly-zeroed open responses** on that quiz (2 students `6e41be06…`, `a599fe24…`; tf_with_reason ×2 + short_answer ×2) to pending — left the genuine keyed multiple_select 0/1 and correct objective rows untouched. NEW check `scripts/test-teacher-quiz-grading.ts` (16 assertions, all pass) simulates a grader failure and asserts no zero is written. `tsc --noEmit` clean (only the pre-existing `registry.test.ts`/vitest noise).

**PHASE 4 — SECURITY:** The four named SECURITY DEFINER fns (`record_trial_result`, `submit_live_quiz_answer`, `finalize_live_quiz_for_me`, `join_live_quiz`) carried a blanket **PUBLIC EXECUTE** grant (raw acl `=X/postgres`) → anon could call them. No legit anon path exists (record_trial_result sits behind a 401; the 3 live-quiz RPCs are called only by JWT-authenticated enrolled students; all four already raise `NOT_AUTHENTICATED`). Migration `revoke_anon_execute_on_quiz_security_definer_fns` revoked EXECUTE from PUBLIC + anon, kept authenticated + service_role → verified anon=false, authenticated=true on all four. Advisor also flagged 2 ERROR `rls_disabled_in_public` = my Phase-2 quarantine tables → migration `enable_rls_on_phase2_quarantine_tables` enabled RLS (no policy = service-role-only; anon/authenticated denied) — cleared both ERRORs.
- **Left deliberately (reported, why):** (1) `mirror_grade_to_history()` + `notify_message_recipients()` still anon-executable SECURITY DEFINER, but they RETURN trigger → Postgres refuses direct `/rest/v1/rpc` calls, so the grant is inert; out of the named scope. (2) `rls_enabled_no_policy` INFO tables (stripe_events, digest_runs, cover_archive, …) — pre-existing internal, per instructions untouched. (3) `demo_requests` INSERT policy WITH CHECK true — intentional (anon demo submissions allowed by design; SELECT locked), per CLAUDE.md.
- **HUMAN TODO:** `auth_leaked_password_protection` = **disabled** (WARN). Flip it on in the Supabase dashboard (Auth → Password security → enable HaveIBeenPwned check) — a dashboard toggle, not attempted here.

**Migrations applied to live DB this session:** `revoke_anon_execute_on_quiz_security_definer_fns`, `enable_rls_on_phase2_quarantine_tables`, plus the Phase-3 `graded_by='pending'` check-constraint extension (applied earlier this task). All additive/reversible. Quarantine tables hold 3 quizzes + 202 questions, restorable at any time.
