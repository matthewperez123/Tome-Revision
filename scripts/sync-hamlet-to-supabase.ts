#!/usr/bin/env npx tsx
/**
 * sync-hamlet-to-supabase.ts — Upsert content/hamlet/* into Supabase
 *
 * Reads the 20 scene JSONs plus _meta.json and upserts into:
 *   works, sections, lines, stage_directions, glosses, annotations, trials
 *
 * Usage:
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npx tsx scripts/sync-hamlet-to-supabase.ts
 *   npx tsx scripts/sync-hamlet-to-supabase.ts --dry-run   # preview, no writes
 */
...
async function main() {
  ...
  console.log(`=== Hamlet Supabase Sync ===`);
