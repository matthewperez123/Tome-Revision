#!/usr/bin/env npx tsx
/**
 * gen-henry-v-sync.ts — Emit Henry V sync SQL into content/henry-v/sync.sql
 * Same pattern as generate-sync-sql.ts (the Hamlet-specific version) but
 * parameterized on 'henry-v' and 'henry_v_'.
 */
import * as fs from "fs";
import * as path from "path";

const ROOT = path.resolve(__dirname, "..");
const WORK_ID = "henry-v";
const PREFIX = "henry_v_"; // filename and section_id prefix
const CONTENT_DIR = path.join(ROOT, `content/${WORK_ID}`);
...
// Clear existing Henry V content for fresh sync
sql.push(`DELETE FROM lines WHERE section_id LIKE '${PREFIX}%';`);
sql.push(`DELETE FROM stage_directions WHERE section_id LIKE '${PREFIX}%';`);
sql.push(`DELETE FROM glosses WHERE section_id LIKE '${PREFIX}%';`);
sql.push(`DELETE FROM scene_annotations WHERE section_id LIKE '${PREFIX}%';`);
sql.push(`DELETE FROM trials WHERE section_id LIKE '${PREFIX}%';`);
sql.push(`DELETE FROM sections WHERE work_id = '${WORK_ID}';`);

for (const s of sections) {
  sql.push(`INSERT INTO sections (id, work_id, sequence, act, scene, scene_title, line_count, word_count, est_read_minutes) VALUES (${esc(s.section_id)}, ${esc(s.work_id)}, ${s.sequence}, ${s.act === null ? "NULL" : s.act}, ${s.scene === null ? "NULL" : s.scene}, ${esc(s.scene_title)}, ${s.line_count}, ${s.word_count}, ${s.est_read_minutes});`);
}
