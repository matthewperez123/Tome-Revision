...
export interface StudyGroupsDirectoryAPI {
  /** "real" when signed in; "demo" otherwise → true empty state. */
  mode: "real" | "demo"
  loading: boolean
  myGroups: StudyGroupSummary[]
  discover: StudyGroupSummary[]
...
/** Directory of study groups: the ones I'm in + public ones to discover. */
export function useStudyGroups(): StudyGroupsDirectoryAPI {
  ...
  const refresh = useCallback(async () => {
    ...
    // RLS limits this to visible groups: all public groups + any I belong to.
    const { data: groupRows } = await supabase
      .from("groups")
      .select("id, slug, name, description, privacy, member_limit, created_at")
      .eq("kind", "study_group")
  ...
  return useMemo(
    ...
    [isReal, loading, myGroups, discover, refresh],
  )
}
