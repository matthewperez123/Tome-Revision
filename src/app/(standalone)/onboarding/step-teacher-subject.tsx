...
export function StepTeacherSubject({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  const [selected, setSelected] = useState<string | null>(null)

  function handleContinue() {
    if (!selected) return
    saveOnboardingData({ teacherSubject: selected })
    onNext()
  }

  return (
    <div className="flex flex-col items-center text-center" role="radiogroup" aria-label="What do you teach?">
      <h2
      ...
      <div className="mt-8 grid w-full grid-cols-2 gap-2">
        {subjects.map((subj) => {
          const isSelected = selected === subj
          return (
            <motion.button
              key={subj}
              role="radio"
