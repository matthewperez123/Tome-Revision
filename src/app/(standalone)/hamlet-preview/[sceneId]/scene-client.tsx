...
/**
 * Apply glosses to a line's text — wrap matched phrases in <span> with
 * an inline tooltip on hover. Longest-phrase-first to avoid substring overlap.
 */
...
export function SceneClient({
  ...
  // Group glosses by line number for O(1) lookup
  ...
  // Group stage directions by after_line
  ...
  // Group annotations by starting line for the margin monogram
  ...
  // Build the render order: walk lines, insert stage directions before line N
  // (i.e. stage dirs with after_line = N-1 appear just before line N).
  // Also track shared-line grouping: lines with the same `number` share a row.
  ...
  for (let i = 0; i < section.lines.length; i++) {
    ...
    // Emit any stage directions that come after the previous line number
    // but we haven't emitted yet (for line numbers that weren't real lines,
    // stage-dir after_line values persist).
    ...
    // Is this line the start of a shared-line pair?
    ...
    // Emit stage dirs immediately after this line (but only if next line has a different number)
    if (!isFirstOfShared) {
  ...
  return (
    <div className="preview-page">
      <nav className="scene-nav">
        <Link href="/hamlet-preview" className="nav-link">← All scenes</Link>
        <div className="scene-title">
          <span className="scene-id">{section.act}.{section.scene}</span>
          <span className="scene-loc">{section.scene_title}</span>
        </div>
        <div className="nav-cluster">
          {prev && (
            <Link href={`/hamlet-preview/${prev.section_id}`} className="nav-link">
              ← {prev.act}.{prev.scene}
            </Link>
          )}
          {next && (
            <Link href={`/hamlet-preview/${next.section_id}`} className="nav-link">
              {next.act}.{next.scene} →
            </Link>
          )}
        </div>
      ...
      <style jsx global>{`
        ...
        .verse-line.shared-continuation {
          padding-top: 0;
        }
        .line-number {
          font-size: 11px;
